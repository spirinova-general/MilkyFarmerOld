 ...
 */
public class CityMapTableManagement {
    ...
    public static ArrayList<VAreaMapper> getCityByAreaId(SQLiteDatabase db, final String areaId, final String accountId) {
    ...
    public static VAreaMapper getAreaNameById(SQLiteDatabase db, final String areaId) {
    ...
    public static VAreaMapper getCityNameById(SQLiteDatabase db, final String cityId) {
        String selectquery = "SELECT * FROM " + TableNames.TABLE_AREA + " WHERE " + TableColumns.CITY_ID + " ='" + cityId + "'";
        Cursor cursor = db.rawQuery(selectquery, null);
        VAreaMapper holder = new VAreaMapper();
        if (cursor.moveToFirst()) {
            do {
